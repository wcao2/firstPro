...
public class FileServiceTest {
    ...
    public void uploadFileTest(){
        ...
        //when(testFile.getName()).thenReturn("sampleFile.txt");//总返回这个 不管怎么调用 为了执行接下来的语句
        fileService.uploadFile(testFile);
        //verify 基于mock的对象 而不是fileservice 里的s3Client
        verify(s3Client,times(1)).putObject(any(PutObjectRequest.class));//spies

        //1
//        File testFile=new File("/home/weicao/Downloads/sampleFile.txt");
//        fileService.uploadFile(testFile);
    }
